/**
 * This code is based on the cl_cbs github repository by author Licheng Wen (wenlc@zju.edu.cn). @date 2020-11-12
 * The code has been modified for usage by Nithish Ravikkumar and Prakul Balaji, IIT ROORKEE
 */
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& node : map_config["agents"]) {
    const auto& start = node["start"];
    const auto& goal = node["goal"];
    State startState(start[0].as<double>(), start[1].as<double>(), start[2].as<double>());
    State goalState(goal[0].as<double>(), goal[1].as<double>(), goal[2].as<double>());

  // Read checkpoints
  const auto& checkpointsNode = node["checkpoints"];
  for (const auto& checkpoint : checkpointsNode) {
    startState.checkpoints.emplace_back(Location(checkpoint[0].as<double>(), checkpoint[1].as<double>()));
  }

  startStates.emplace_back(startState);
  goals.emplace_back(goalState);
}
